...
enum AppLogCategory: String, Codable, CaseIterable, Identifiable {
    ...
    fileprivate var osLogger: Logger {
        Logger(subsystem: "com.quicktranslate", category: title)
    }
}
...
final class AppLogStore: ObservableObject {
    ...
    private let fileQueue = DispatchQueue(label: "com.quicktranslate.applog.file", qos: .utility)

    private init() {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? FileManager.default.temporaryDirectory
        let dir = support.appendingPathComponent("QuickTranslate", isDirectory: true)
